...
RecTy *mini_tablegen::resolveTypes(RecTy *T1, RecTy *T2) {
  if (T1 == T2)
    return T1;
    ...
    if (isa<StringRecTy>(getType())) {
      ...
      if (StringInit *Name = dyn_cast<StringInit>(LHS)) {
        ...
        if (!DI->getType()->typeIsA(getType())) {
          PrintFatalErrorHelper(
              Twine("Expected type '") + getType()->getAsString() + "', got '" +
              DI->getType()->getAsString() + "' in: " + getAsString() + "\n");
        }
        return DI;
      }
    }
...
Init *CondOpInit::Fold(Record *CurRec) const {
  ...
  PrintFatalError(
      ...
      CurRec->getNameInitAsString() +
          " does not have any true condition in:" + this->getAsString());
  return nullptr;
}
...
raw_ostream &mini_tablegen::operator<<(raw_ostream &OS, const Record &R) {
...
raw_ostream &mini_tablegen::operator<<(raw_ostream &OS, const RecordKeeper &RK) {
  OS << "------------- Classes -----------------\n";
